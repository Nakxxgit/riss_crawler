...
def load_journal_kr(driver, page, keyword):

    url = 'http://www.riss.kr/search/Search.do?detailSearch=false&viewYn=OP&queryText=&iGroupView=5&icate=all&colName=re_a_kor&pageScale=1000&strSort=RANK&order=%2FDESC&query='
    url2 = '&iStartCount='
...
if __name__ == '__main__':
    ...
    driver.get('http://www.riss.kr/search/Search.do?detailSearch=false&viewYn=OP&queryText=&iGroupView=5&icate=all&colName=re_a_kor&pageScale=1000&strSort=RANK&order=%2FDESC&query='+keyword+'&iStartCount='+str(0))
    ...
    for i in range(ceil(results / 1000)):
        print(str(i * 1000))
        load_journal_kr(driver, i, keyword)
