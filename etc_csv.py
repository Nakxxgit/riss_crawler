...
def convert_to_csv():
    ...
    for f in glob.glob('./etc/*.xls'):
        ...
        new_df = pd.DataFrame({'제목': title, '저자': author, '학술지명': 'NaN', '권호사항': 'NaN', '발행처': where, '자료유형': doc_type, '수록면': 'NaN', '발행년도': year, 'KDC': kdc, '초록': abstract})
    ...
    return full_df
    #full_df.to_csv('etc.csv', sep=',', na_rep='NaN', encoding = 'utf-8')